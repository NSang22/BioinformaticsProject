...
dist_genes = dist(expression_top5000, method="euclidean")
...
# create hclust_genes dataframe
hclust_genes_df = hclust_genes$height %>%
            as.tibble() %>%
            add_column(groups = length(hclust_genes$height):1)
# create hclust_samples dataframe
hclust_samples_df = hclust_samples$height %>%
            as.tibble() %>%
            add_column(groups = length(hclust_samples$height):1)

hg_df_plot = ggplot(hclust_genes_df,
...
hg_df_plot +
  ...
  geom_abline(intercept = 680, slope = -1, color = "blue")
hs_df_plot + 
