...
```{r "Install Packages", message=FALSE, eval = TRUE, cache=FALSE}
...
# Section 1
```{r "Load Data", message=FALSE, eval = TRUE, cache=FALSE}
...
# Section 2a
```{r "Select Top Genes", eval = TRUE, cache=FALSE}
...
```{r "Cluster Determination pt3", cache=FALSE}
library(tidyverse)
...
heatmap <- Heatmap(
    ...
    column_title = paste0("Patient Clustering Axis: Molecular Subtypes (k=2) by Prior Dengue Status (N=", ncol(expression_top5000), ")"),
...
I applied Partitioning Around Medoids (PAM) clustering to the top 5000 most variable genes, testing k values from 2 to 8. 
The average silhouette width was highest for k = 2 (0.099), indicating two primary sample groups. T
his aligns with the k-means results but with lower sensitivity to outliers due to PAM’s medoid-based approach. 
The clusters contained 676 and 345 samples, respectively.
When k increased beyond 2, the silhouette scores decreased at a steady rate, and it seemed that additional clusters were not biologically meaningful in terms of results. 
Specificaly, the silhouette scores went from 0.999 at k = 2 to 0.065 at k = 8, showing that the clusters became less distinct. 
So, I chose k = 2 samples to strike the optimal balance. 
I then reran PAM with 10, 100, 1000, and 10000 genes, which took varying amounts of time, but the k value of 2 stayed consistent.
With small gene sets, the cluster boundaries were more unstable, and with larger sets, the clusters stabilized.
I also performed chi-squared tests; here is the table
Table: Chi-squared test results for PAM clustering across gene subsets
...
This shows that as the number of genes increased, the p values decreased, except with a very small number of genes, 10 to be exact where there was not enough data.
I also generated a heatmap with hierarchical row and column dendrograms and annotations for both PAM clusters (k = 2) and exposure groups from Assignment 1. 
The z-scored expression profiles display clear global differences between the two PAM clusters, though overlap is visible across some samples.
After running PAM for k = 2 - 8 across the 5 000 most variable genes, k = 2 yielding the highest average silhouette (about 0.10).
Repeating the analysis for 10 - 10 000 genes revealed similar results: silhouette scores declined gradually as dimensionality increased, but the optimal k remained 2.
Heatmaps of the top 5 000 genes showed two major expression clusters that partially overlapped with exposure groups.
The chi-squared tests confirmed that these two clusters were statistically associated with exposure in most gene subsets.
...
```{r spectral-5000-genes, cache=FALSE, cache.lazy=FALSE}
...
```{r spectral-multiple-genes, cache=FALSE, cache.lazy=FALSE}
...
```{r spectral-assignment1-comparison, cache=FALSE, cache.lazy=FALSE}
...
```{r spectral-adjust-pvalues, cache=FALSE}
...
```{r combined-cluster-heatmap, cache=FALSE}
suppressPackageStartupMessages({
  library(pheatmap)
  library(cluster)
  library(kernlab)
...
sample_order <- colnames(expression_top5000)

log_mat <- log2(expression_top5000 + 1)
scaled_mat <- t(scale(t(log_mat)))
scaled_mat[!is.finite(scaled_mat)] <- 0
scaled_mat <- pmax(pmin(scaled_mat, 4), -4)

hm_colors <- colorRampPalette(c("navy", "white", "firebrick3"))(101)
heatmap_breaks <- seq(-4, 4, length.out = length(hm_colors) + 1)

if (!exists("hclust_samples")) {
  dist_mat <- dist(t(expression_top5000), method = "euclidean")
  hclust_samples <- hclust(dist_mat, method = "ward.D2")
}
hier_clusters <- factor(cutree(hclust_samples, k = 2)[sample_order])

kmeans_clusters_vec <- NULL
if (exists("kmeans_assignments_5000")) {
  kmeans_clusters_vec <- kmeans_assignments_5000$cluster[match(sample_order, kmeans_assignments_5000$sample_id)]
}
if (is.null(kmeans_clusters_vec) || any(is.na(kmeans_clusters_vec))) {
  set.seed(0L)
  km_fit <- kmeans(t(scaled_mat), centers = 2, nstart = 50)
  kmeans_clusters_vec <- km_fit$cluster[sample_order]
}
kmeans_clusters <- factor(kmeans_clusters_vec)

pam_input <- t(scaled_mat)
rownames(pam_input) <- sample_order

pam_clusters_vec <- NULL
if (exists("pam_clusters") && all(sample_order %in% names(pam_clusters))) {
  pam_clusters_vec <- pam_clusters[sample_order]
} else if (exists("pam_clusters_5000")) {
  if (is.data.frame(pam_clusters_5000) && all(sample_order %in% pam_clusters_5000$sample_id)) {
    pam_clusters_vec <- pam_clusters_5000$cluster[match(sample_order, pam_clusters_5000$sample_id)]
  } else if (is.vector(pam_clusters_5000) && all(sample_order %in% names(pam_clusters_5000))) {
    pam_clusters_vec <- pam_clusters_5000[sample_order]
  }
}
if (is.null(pam_clusters_vec) || any(is.na(pam_clusters_vec))) {
  pam_fit <- pam(pam_input, k = 2)
  pam_clusters_vec <- pam_fit$clustering[sample_order]
}
pam_clusters <- factor(pam_clusters_vec)

if (exists("spectral_clusters_5000_best")) {
  spectral_clusters_vec <- spectral_clusters_5000_best[sample_order]
} else {
  spectral_fit <- specc(pam_input, centers = 2)
  spectral_clusters_vec <- spectral_fit@.Data
  names(spectral_clusters_vec) <- rownames(pam_input)
  spectral_clusters_vec <- spectral_clusters_vec[sample_order]
}
if (any(is.na(spectral_clusters_vec))) {
  spectral_fit <- specc(pam_input, centers = 2)
  spectral_clusters_vec <- spectral_fit@.Data
  names(spectral_clusters_vec) <- rownames(pam_input)
  spectral_clusters_vec <- spectral_clusters_vec[sample_order]
}
spectral_clusters <- factor(spectral_clusters_vec)

metadata_ids <- as.character(metadata$refinebio_accession_code)
exposure_vec <- as.character(metadata$Exposure)[match(sample_order, metadata_ids)]
exposure_vec[is.na(exposure_vec)] <- "Unknown"
exposure <- factor(exposure_vec)
...
palette_for <- function(fct, palette_fun) {
  lvl <- levels(fct)
  setNames(palette_fun(length(lvl)), lvl)
}

ann_colors <- list(
  KMeans = palette_for(kmeans_clusters, hue_pal()),
  Spectral = palette_for(spectral_clusters, hue_pal(l = 65, c = 100)),
  PAM = palette_for(pam_clusters, hue_pal(l = 60, c = 90)),
  Hierarchical = palette_for(hier_clusters, hue_pal(l = 55, c = 85)),
  Exposure = palette_for(exposure, hue_pal())
)
...
combined_heatmap <- pheatmap(
  ...
  main = "Top 5000 genes with multi-method cluster annotations"
)
...
pheatmap(
  ...
  clustering_method = "ward.D2",
  main = "Top 5000 genes with multi-method cluster annotations",
  filename = file.path("plots", "combined_clustering_heatmap.png"),
  width = 12,
  height = 10
)
```
