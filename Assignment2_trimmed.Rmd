...
if (!requireNamespace("GenomicSuperSignature", quietly = TRUE)) {
  if (!requireNamespace("BiocManager", quietly = TRUE)) install.packages("BiocManager", repos = getOption("repos")["CRAN"])
...
if (length(gss_indices) > 0) {
  ...
  # Try to get descriptive labels from annotateRAV() if it returns a data.frame
  ...
  if (is.data.frame(ann_try)) {
    ...
    if (length(desc_cols) >= 1) {
      # take the first descriptive column available
      desc_col <- desc_cols[1]
      # if rows correspond to validated_ind order, map directly; otherwise fill best-effort
