...
if (!exists("expression_df") || !exists("merged_meta")) {
  sample_limit_local <- if (exists("sample_limit")) sample_limit else 200
...
if (nrow(summary_results) > 0) {
  ...
  # Create top 200 results
  top_results <- summary_results %>%
    slice_head(n = 200)
  write_tsv(top_results, file.path(results_dir, "GO_enrichment_top200_results.tsv"))
