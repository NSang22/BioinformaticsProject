...
Please do not run this more than once it takes so long.
```{r Data Wrangling}
...
# convert exposure to a factor
coldata <- coldata %>%
  dplyr::mutate(
    exposure = factor(exposure, levels = c("naive", "DENV"))
  )

# Set the sample names as rownames for colData
coldata_final <- coldata %>%
  tibble::column_to_rownames("sample")

# Filter the count matrix columns to match the samples in coldata_final
cts_matrix_final <- cts_matrix[, rownames(coldata_final)]

# FINAL CHECK: Ensure column names in the count matrix match row names in colData
# This should now return TRUE
all.equal(colnames(cts_matrix_final), rownames(coldata_final))
cts_matrix_integer <- round(cts_matrix_final)
# Start DEseq
dds <- DESeqDataSetFromMatrix(countData = cts_matrix_integer,
