...
if (exists("res_df")) {
  ...
  top_hits <- res_annot %>%
    ...
    mutate(label = ifelse(!is.na(Symbol) & Symbol != "", Symbol, GeneID)) %>%
    mutate(summary = sprintf("%s (log2FC = %.2f, padj = %.2g)", label, log2FoldChange, padj))
...
go_data_taylor <- topGO::new(
  "topGOdata",
  description = "Taylor Tillander GO BP",
  ontology = "BP",
  allGenes = gene_scores,
  geneSelectionFunc = selection_fun,
  nodeSize = 10,
  annot = annFUN.org,
...
result_classic <- topGO::runTest(go_data_taylor, algorithm = "classic", statistic = "fisher")
result_weight01 <- topGO::runTest(go_data_taylor, algorithm = "weight01", statistic = "fisher")
...
go_table_taylor <- topGO::GenTable(
  go_data_taylor,
...
assign("topgo_results_taylor", go_table_taylor, envir = .GlobalEnv)

if (!dir.exists(results_dir)) dir.create(results_dir, recursive = TRUE)
readr::write_tsv(go_table_taylor, file.path(results_dir, "topGO_BP_TaylorTillander.tsv"))

topgo_top10 <- go_table_taylor %>% slice_head(n = 10)
assign("topgo_top10_taylor", topgo_top10, envir = .GlobalEnv)
readr::write_tsv(topgo_top10, file.path(results_dir, "topGO_BP_TaylorTillander_top10.tsv"))
...
topgo_plot <- topgo_top10 %>%
  ...
  labs(
    title = "topGO BP enrichment (weight01 Fisher) — Taylor Tillander",
    x = "-log10(weight01 Fisher p-value)",
    y = NULL
  ) +
  theme_minimal()

plot_path <- file.path(plots_dir, paste0(run_label, "_topgo_BP_TaylorTillander.png"))
ggsave(plot_path, topgo_plot, width = 8, height = 5, dpi = 300)
topgo_plot
```

```{r echo=FALSE}
if (exists("topgo_results_taylor")) {
  total_terms <- nrow(topgo_results_taylor)
  sig_terms <- sum(topgo_results_taylor$weight01Fisher_p < 0.05, na.rm = TRUE)
  highlight <- topgo_results_taylor %>%
    arrange(weight01Fisher_p) %>%
    slice_head(n = 3) %>%
    transmute(label = sprintf("%s (weight01 p = %.2g)", Term, weight01Fisher_p))

  cat(sprintf("Taylor Tillander's topGO Biological Process analysis tested %d GO terms and found %d passing the 0.05 weight01 Fisher threshold.",
...
if (exists("topgo_results_taylor")) {
  cat(sprintf("Taylor Tillander's topGO run contributed %d Biological Process terms (with %d passing weight01 < 0.05), and the summaries were written alongside the clusterProfiler outputs.\n",
              nrow(topgo_results_taylor),
              sum(topgo_results_taylor$weight01Fisher_p < 0.05, na.rm = TRUE)))
}
```
...
# topGO or other team methods saved to results directory
